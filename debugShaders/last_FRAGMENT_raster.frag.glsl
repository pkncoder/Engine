...
const vec3  fogColor = vec3(0.1, 0.4, 0.8);
const float fogDensity = 0.03;
const bool fog = true;

const float vignetteRadius = 2.5;
const float vignetteSoftness = 0.9;
const bool vignette = true;

// END INCLUDE: ../stylizing.glsl
...
void main() {
    ...
    vec3 localNormal;
    if (uIsBumpMap == 1) {
        localNormal = bumpToTangentNormal(uBumpMap, vTexCoords, vWorldPos, 0.06);
    } else {
